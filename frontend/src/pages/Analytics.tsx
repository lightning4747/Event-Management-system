...
export const Analytics: React.FC = () => {
  ...
  const [activityCategory, setActivityCategory] = React.useState('');
  const [activityType, setActivityType] = React.useState('');
  const [viewMode, setViewMode] = React.useState<'general' | 'specific'>('general');

  // Handle category change -> reset activity type if incompatible
  const handleCategoryChange = (val: string) => {
    setActivityCategory(val);
    if (val === 'Extracurricular' && !extracurricularTypes.includes(activityType)) {
      setActivityType('');
    } else if (val === 'Co-curricular' && !cocurricularTypes.includes(activityType)) {
      setActivityType('');
    }
  };
  ...
  const handleActivityTypeChange = (val: string) => {
    ...
    if (val) {
      if (extracurricularTypes.includes(val)) {
        setActivityCategory('Extracurricular');
      } else if (cocurricularTypes.includes(val)) {
        setActivityCategory('Co-curricular');
      }
    }
  };
  ...
  const { data, isLoading, isError, error } = useQuery<AnalyticsData>({
    ...
    queryFn: async () => {
      ...
      if (selectedYear) params.append('admissionYear', selectedYear);
      if (section) params.append('section', section);
      if (activityCategory) params.append('activityCategory', activityCategory);
    ...
    <DashboardShell>
      <div className="space-y-8 pb-16 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-200 pb-5">
          ...
          <div className="bg-slate-100 p-1 rounded-xl flex items-center gap-1 self-start md:self-auto border border-slate-200">
            <button
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-3">
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <Button
              ...
              variant="outline"
              className="h-7 px-2.5 text-xs font-semibold text-slate-600 hover:text-blue-600 hover:bg-blue-50 border-slate-200"
            >
        ...
        {!isLoading && !isError && (
          <>
            ...
            <div className="space-y-4">
              <h2 className="text-sm font-semibold text-slate-800 tracking-tight uppercase">
                {viewMode === 'general' ? 'Activity Category Distribution' : 'Activity Type Drilldown'}
              </h2>
